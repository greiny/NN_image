...
using namespace cv;
using namespace std;
std::fstream costFile;

void costLogging(const char* filename){
	//create log file 
	if ( ! costFile.is_open() )
	{
		costFile.open(filename, ios::out);
		if ( costFile.is_open() )
		costFile << "Epoch,Iter,J1,J2,Cost," << endl;
	}
}
...
//    deltad2[deltad2.size() - 1] = deltad2[deltad2.size() -1].mul(pow(dsigmoid_a(acti[acti.size() -1]), 2.0));
    ...
    for(int cl = CLayers.size() - 1; cl >= 0; cl --){
        ...
        for(int k = 0; k < deltaKey.size(); k ++){
            ...
            Mat upDelta = UnPooling(cpmap.at(deltaStr), pDim, pDim, pMtd, locmap.at(locstr));
            Mat upHessian = UnPooling(cpmap.at(hessianStr), pDim, pDim, pMtd, locmap.at(locstr));
...
void
getNetworkCost(vector<Mat> &x, Mat &y, vector<Cvl> &CLayers, vector<Fcl> &hLayers, Smr &smr, int epoch, int iter){
    ...
    vector<string> vecstr = getLayerKey(nsamples, CLayers.size() - 1, KEY_POOL);
    for(int i = 0; i<vecstr.size(); i++){
	//cout <<cpmap.at(vecstr[i]).cols <<"x "<<cpmap.at(vecstr[i]).rows<<endl;
        P.push_back(cpmap.at(vecstr[i]));
    }
...
//      tmpacti = sigmoid(tmpacti);
	...
	if(iter%10==0) costFile << epoch << "," << iter << "," << J1 << "," << J2 << "," << smr.cost << endl;
    }
